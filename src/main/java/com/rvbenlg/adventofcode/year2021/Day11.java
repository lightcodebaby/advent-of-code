...
public class Day11 {
    ...
    private Octopus[][] octopuses = new Octopus[0][0];
    ...
    private int whenAllTheOctopusesToFlashSimultaneously() {
        ...
        do {
            increaseOctopusesEnergy();
            flash();
            howManyOctopusesFlashed = howManyOctopusesFlashed();
            updateOctopusesEnergy();
            result++;
        } while (howManyOctopusesFlashed != octopuses.length * octopuses[0].length);
        return result;
    }

    private int takeSteps() {
        int result = 0;
        for (int i = 0; i < 100; i++) {
            increaseOctopusesEnergy();
            flash();
            result += howManyOctopusesFlashed();
            updateOctopusesEnergy();
        }
        return result;
    }

    private void increaseOctopusesEnergy() {
        for (int i = 0; i < octopuses.length; i++) {
            for (int j = 0; j < octopuses[i].length; j++) {
                if (!octopuses[i][j].hasFlashed) {
                    octopuses[i][j].energyLevel = octopuses[i][j].energyLevel + 1;
                }
            }
        }
    }
    ...
    private int howManyOctopusesFlashed() {
        int result = 0;
        for (int i = 0; i < octopuses.length; i++) {
            for (int j = 0; j < octopuses[i].length; j++) {
    ...
    private void parseOctopuses(List<String> input) {
        octopuses = new Octopus[input.size()][];
        for (int i = 0; i < input.size(); i++) {
            String line = input.get(i);
            octopuses[i] = new Octopus[line.length()];
            for (int j = 0; j < line.length(); j++) {
                octopuses[i][j] = new Octopus(i, j, Integer.parseInt(String.valueOf(line.charAt(j))));
            }
        }
    }

    private void resetVariables() {
        octopuses = new Octopus[0][0];
    }
