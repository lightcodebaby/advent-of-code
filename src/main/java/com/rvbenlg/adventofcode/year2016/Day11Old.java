...
public class Day11Old {
    ...
    private void emptyFirstFloor() {
        Status initStatus = getInitStatus();
        fillElements(initStatus);
        statuses.add(getInitStatus());
        boolean goUp = true;
        while (statuses.stream().noneMatch(status -> status.floors.stream().anyMatch(floor -> floor.generators.size() == elements.size() && floor.microchips.size() == elements.size()))) {
            ...
            for(Status status : toCheck) {
                if(!checkedStatuses.contains(status.toHash())) {
                    checkGeneratorsAndMicrochips(status, goUp);
                    checkOnlyGenerators(status, goUp);
                    checkOnlyMicrochips(status, goUp);
...
//        return steps;
    ...
    private boolean checkGeneratorsAndMicrochips(Status currentStatus, boolean goUp) {
        ...
        for (String generator : currentStatus.floors.get(currentStatus.elevator).generators) {
            ...
            for (String microchip : currentStatus.floors.get(currentStatus.elevator).microchips) {
                ...
                if (goUp && canGoUp(currentStatus, generatorsToBring, microchipsToBring)) {
                    result = true;
                    Status nextStatus = goUp(currentStatus, generatorsToBring, microchipsToBring);
                    if (!previousStatus(nextStatus) && isValidStatus(nextStatus)) {
                        statuses.addAll(notPreviousStatuses(equivalentStatuses(nextStatus)));
                    }
                }
                if (!goUp && canGoDown(currentStatus, generatorsToBring, microchipsToBring)) {
                    result = true;
    ...
    private void checkOnlyMicrochips(Status currentStatus, boolean goUp) {
        ...
        for (int i = 0; i < microchips.size(); i++) {
            ...
            for (int j = i + 1; j < microchips.size(); j++) {
                ...
                if (goUp && canGoUp(currentStatus, new ArrayList<>(), microchipsToBring)) {
                    Status nextStatus = goUp(currentStatus, new ArrayList<>(), microchipsToBring);
                    if (!previousStatus(nextStatus) && isValidStatus(nextStatus)) {
                        statuses.addAll(notPreviousStatuses(equivalentStatuses(nextStatus)));
                    }
                }
                if (!goUp && canGoDown(currentStatus, new ArrayList<>(), Collections.singletonList(microchip))) {
            ...
            if (goUp && canGoUp(currentStatus, new ArrayList<>(), Collections.singletonList(microchip))) {
    ...
    private void checkOnlyGenerators(Status currentStatus, boolean goUp) {
        ...
        for (int i = 0; i < generators.size(); i++) {
            ...
            for (int j = i + 1; j < generators.size(); j++) {
                ...
                if (goUp && canGoUp(currentStatus, generatorsToBring, new ArrayList<>())) {
                    Status nextStatus = goUp(currentStatus, generatorsToBring, new ArrayList<>());
                    if (!previousStatus(nextStatus) && isValidStatus(nextStatus)) {
                        statuses.addAll(notPreviousStatuses(equivalentStatuses(nextStatus)));
                    }
                }
                if (!goUp && canGoDown(currentStatus, Collections.singletonList(generator), new ArrayList<>())) {
            ...
            if (goUp && canGoUp(currentStatus, Collections.singletonList(generator), new ArrayList<>())) {
    ...
    private List<Status> equivalentStatuses(Status nextStatus) {
        List<Status> equivalentStatuses = new ArrayList<>();
            ...
            for (int j = 0; j < elements.size(); j++) {
                if (i != j) {
                    ...
                    if (!checkedStatuses.contains(auxStatus.toHash())) {
                        equivalentStatuses.add(auxStatus);
                    }
                }
            }
        }
        return equivalentStatuses;
    }
    ...
    private List<Status> notPreviousStatuses(List<Status> nextStatuses) {
        return nextStatuses.stream().filter(status -> !checkedStatuses.contains(status.toHash()) && statuses.stream().noneMatch(status1 -> status1.toHash().equals(status.toHash()))).collect(Collectors.toList());
    }
...
//            }
    ...
    private boolean isValidStatus(Status nextStatus) {
        boolean result = nextStatus.floors.stream().noneMatch(floor -> floor.microchips.stream().anyMatch(microchip -> !floor.generators.isEmpty() && !floor.generators.contains(microchip)));
        return result;
    }
    ...
    private boolean isValidDestination(Floor destination, List<String> generators, List<String> microchips) {
        ...
        for (String generator : generators) {
            if (destination.microchips.stream().anyMatch(microchip -> !microchip.equals(generator) && !destination.generators.contains(microchip) && !generators.contains(microchip))) {
                result = false;
            }
